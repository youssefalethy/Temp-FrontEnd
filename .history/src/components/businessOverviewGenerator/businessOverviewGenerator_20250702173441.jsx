  ...
  const onFinish = async (value) => {
    ...
    try {
      ...
      setOverviewData({
        ...
        strategy: business_overview?.strategy?.map(item => 
          typeof item === 'string' ? item : `${item.initiative}: ${item.description}`
        ),
        unique_selling_points: business_overview?.unique_selling_points?.map(item => 
          typeof item === 'string' ? item : `${item.initiative}: ${item.description}`
        ),
      });
      setDataAdded(true);
